...
// คัดลอกข้อมูลมาจาก airth-frontend/src/data/provinces.ts
// (ทั้ง 77 จังหวัด — วางไว้ตรงนี้)
const provinces: SeedProvince[] = [
...
async function seed() {
  ...
  for (const p of provinces) {
    await pool.query(
      `INSERT INTO provinces (id, name_th, name_en, region, lat, lng)
       VALUES ($1, $2, $3, $4, $5, $6)
       ON CONFLICT (id) DO UPDATE SET
         ...
         lat = EXCLUDED.lat,
         lng = EXCLUDED.lng`,
      [p.id, p.nameTh, p.nameEn, p.region, p.lat, p.lng]
    )
  }
